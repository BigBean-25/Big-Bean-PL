...
const ClosingStockUpload = () => {
  ...
  const permissions = useMemo(() => getStoredPermissions()?.closing_stock || {}, []);
  ...
  const runWorkflowAction = async (id, action, body = {}) => {
    ...
    try {
      await uploadAPI[`${action}StockUpload`]("closing_stock", id, body);
      const labels = { submit: "submitted for verification", verify: "verified", reject: "rejected" };
      toast.success(`Closing stock upload ${labels[action] || action}`);
  ...
  return (
    ...
    >
      ...
      {verifyId && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/40 p-4">
          <div className={`w-full max-w-md rounded-md border p-6 shadow-lg ${cardClass}`}>
            <h3 className={`text-[18px] font-semibold ${mainTextClass}`}>Verify Closing Stock Upload</h3>
      ...
      {rejectId && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/40 p-4">
          <div className={`w-full max-w-md rounded-md border p-6 shadow-lg ${cardClass}`}>
            <h3 className={`text-[18px] font-semibold ${mainTextClass}`}>Reject Closing Stock Upload</h3>
