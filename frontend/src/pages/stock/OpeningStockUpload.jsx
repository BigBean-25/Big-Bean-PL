...
const OpeningStockUpload = () => {
  ...
  const fetchUploads = async () => {
    try {
      const response = await uploadAPI.getUploadHistory("opening_stock");
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      await uploadAPI.uploadOpeningStock(uploadData);
      toast.success("Opening stock uploaded successfully");
  ...
  const handleDownloadOriginal = async (upload) => {
    ...
    try {
      const response = await uploadAPI.downloadOpeningStockOriginal(upload.id);
      triggerBlobDownload(response.data, upload.file_name || `opening-stock-${upload.id}.xlsx`);
    } catch (error) {
  ...
  const handleDownloadProcessed = async (upload) => {
    ...
    setDownloadingKey(key);
    try {
      const response = await uploadAPI.downloadOpeningStockProcessed(upload.id);
      triggerBlobDownload(response.data, `opening-stock-processed-${upload.id}.xlsx`);
    } catch (error) {
  ...
  const handleDownloadErrors = async (upload) => {
    ...
    setDownloadingKey(key);
    try {
      const response = await uploadAPI.downloadOpeningStockErrors(upload.id);
      triggerBlobDownload(response.data, `opening-stock-errors-${upload.id}.xlsx`);
    } catch (error) {
  ...
  const handleDownloadTemplate = async () => {
    ...
    try {
      ...
      const response = await uploadAPI.downloadOpeningStockTemplate({ month, year });
      const monthLabel = new Date(2000, month - 1).toLocaleString("default", { month: "long" });
      triggerBlobDownload(response.data, `Opening_Stock_Template_${monthLabel}_${year}.xlsx`);
    } catch (error) {
  ...
  const handleDelete = async (upload) => {
    ...
    const confirmed = window.confirm(
      `Delete "${upload.file_name || "this opening stock upload"}"? This action cannot be undone.`
    );
    ...
    if (typeof uploadAPI.deleteUpload !== "function") {
      toast.error(
        "Delete API is not configured yet. Add uploadAPI.deleteUpload and the backend DELETE route first."
      );
      return;
    }
    ...
    try {
      await uploadAPI.deleteUpload(upload.id, "opening_stock");
      toast.success("Opening stock upload deleted successfully");
    ...
    >
      <div className="flex min-w-0 flex-col justify-between gap-4 xl:flex-row xl:items-center">
        <div className="min-w-0">
          <h1 className={`text-[24px] font-semibold ${mainTextClass}`}>Opening Stock Upload</h1>
          <p className={`mt-1 text-[15px] ${mutedClass}`}>Upload monthly opening stock data via Excel.</p>
        </div>
      ...
      <div className={`min-w-0 rounded-md border p-6 shadow-[0_2px_12px_rgba(47,43,61,0.08)] ${cardClass}`}>
        <div className="mb-6">
          <h3 className={`text-[20px] font-semibold ${mainTextClass}`}>Upload Opening Stock</h3>
        ...
        <form onSubmit={handleUpload} className="space-y-5">
          ...
          <div>
            ...
            >
              ...
              <div className="mt-4 inline-flex items-center gap-2 rounded-md border border-[#DBDADE] bg-white px-4 py-2 text-[13px] font-semibold text-[#5D596C] shadow-sm dark:border-[#4A4F68] dark:bg-[#2F3349] dark:text-[#D0D2D6]">
      ...
      {selectedUpload && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div
            className={`relative z-10 w-full max-w-2xl overflow-hidden rounded-xl border shadow-2xl ${cardClass}`}
          >
            <div
              className={`flex min-w-0 items-start justify-between gap-4 border-b p-5 sm:p-6 ${
            ...
            <div className="max-h-[70vh] overflow-y-auto p-5 sm:p-6">
              <div className="grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2">
                <DetailItem
                  label="Uploaded At"
                  value={formatDate(selectedUpload.created_at)}
                />
                <DetailItem
                  label="Outlet"
                  value={getOutletName(selectedUpload)}
                />
                <DetailItem
                  label="Month / Year"
                ...
                <DetailItem
                  ...
                  value={selectedUpload.status === "Completed" ? selectedUpload.approval_status || "Draft" : "-"}
                />
                <DetailItem
                  label="Submitted At"
                  value={selectedUpload.submitted_at ? formatDate(selectedUpload.submitted_at) : "-"}
                />
                <DetailItem
                  label="Verified At"
                  value={
                ...
                <DetailItem
                  label="Failed Rows"
                  value={selectedUpload.failed_rows ?? 0}
                />
                <DetailItem
                  label="Total Rows"
                  value={selectedUpload.total_rows ?? 0}
                />
                <DetailItem
                  label="File Name"
                  value={selectedUpload.file_name || "-"}
                />
              </div>
              ...
              >
                <p className={`text-[12px] ${mutedClass}`}>
                  View the uploaded batch details or download the original Excel file.
                </p>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => setSelectedUpload(null)}
                    className={`inline-flex h-10 items-center justify-center rounded-md border px-4 text-[14px] font-medium ${cardClass}`}
                  >
                    Close
                  </button>
                </div>
      ...
      <div className={`min-w-0 max-w-full rounded-md border shadow-[0_2px_12px_rgba(47,43,61,0.08)] ${cardClass}`}>
        <div className="border-b border-[#EBE9F1] p-6">
          ...
          <p className={`mt-1 text-[14px] ${mutedClass}`}>Previous opening stock upload records.</p>
        </div>
...
export default OpeningStockUpload;
