...
import { Download, Search, Loader2, Receipt, FileText, AlertCircle } from 'lucide-react';
...
const SalesGSTReport = () => {
  ...
  const handleGenerateReport = async () => {
    ...
    setLoading(true);
    try {
      const r = await reportAPI.getSalesGST(filters);
  ...
  const handleExport = async () => {
    const rows = reportData.map((item) => [
      item.outlet_name || '-',
      item.upload_count,
      fmtINR(item.taxable_value),
      fmtINR(item.tax_amount),
      fmtINR(item.gross_value),
    ]);
    ...
    await exportReportPDF({
      ...
      outletName,
      dateRangeLabel,
      columns: ["Outlet", "Uploads", "Taxable Value", "Tax Amount", "Gross Value"],
      rows,
      summaryLines: [
        `Total Taxable Value: ${fmtINR(totals.taxable)}`,
        `Total Tax Collected: ${fmtINR(totals.tax)}`,
        `Total Gross Value: ${fmtINR(totals.gross)}`,
      ],
      fileName: `sales-gst-${filters.from_date}-to-${filters.to_date}.pdf`,
    });
  ...
  return (
    <div className="page-enter space-y-4 sm:space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className={`text-xl font-bold sm:text-2xl ${mainCls}`}>Sales GST Report</h1>
          <p className={`mt-1 text-[13px] sm:text-[14px] ${mutedCls}`}>Taxable value and tax collected on approved sales uploads, per outlet — a summary, not a GSTR-ready filing export</p>
        </div>
      ...
      {!loading && reportData.length > 0 && (
        <div className={`animate-fade-up flex flex-col gap-3 rounded-md border p-4 shadow-[0_2px_12px_rgba(47,43,61,0.06)] sm:flex-row sm:items-center sm:p-5 ${cardCls}`}>
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className={`text-[13px] ${mutedCls}`}>Total Tax Collected</p>
              <p className="mt-0.5 text-[22px] font-bold text-[#00CFE8]">{fmtINR(totals.tax)}</p>
            </div>
          </div>
      ...
      {!loading && reportData.length > 0 && (
        <div className={`animate-fade-up rounded-md border shadow-[0_2px_12px_rgba(47,43,61,0.06)] ${cardCls}`}>
          <div className={`flex items-center gap-3 border-b px-4 py-3 sm:px-6 ${borderCls}`}>
            ...
            <h3 className={`text-[15px] font-semibold ${mainCls}`}>Sales GST by Outlet</h3>
          </div>
          <div className="overflow-x-auto" style={{ WebkitOverflowScrolling: "touch" }}>
            <table className="min-w-full" style={{ minWidth: "700px" }}>
              <thead>
                <tr>
                  {["Outlet", "Uploads", "Taxable Value", "Tax Amount", "Gross Value"].map((h) => (
              ...
              <tbody className={`divide-y ${borderCls}`}>
                {reportData.map((item, idx) => (
                  <tr key={idx} className={`transition ${trHover}`}>
                    <td className={`px-4 py-3 text-[14px] font-medium ${mainCls}`}>{item.outlet_name || '-'}</td>
                    <td className={`px-4 py-3 text-[14px] tabular-nums ${mutedCls}`}>{item.upload_count}</td>
                    <td className={`px-4 py-3 text-[14px] tabular-nums ${mutedCls}`}>{fmtINR(item.taxable_value)}</td>
                    <td className={`px-4 py-3 text-[14px] font-semibold tabular-nums text-[#00CFE8]`}>{fmtINR(item.tax_amount)}</td>
                ...
                <tr className={isDark ? "bg-[#25293C]" : "bg-[#F8F7FA]"}>
                  <td colSpan="2" className={`px-4 py-3 text-right text-[13px] font-semibold ${mainCls}`}>Total</td>
                  <td className={`px-4 py-3 text-[14px] font-bold ${mainCls}`}>{fmtINR(totals.taxable)}</td>
                  <td className="px-4 py-3 text-[14px] font-bold text-[#00CFE8]">{fmtINR(totals.tax)}</td>
      ...
      {!loading && hasGenerated && reportData.length === 0 && (
        <div className={`flex flex-col items-center justify-center rounded-md border py-14 px-4 text-center ${cardCls}`}>
          ...
          <p className={`text-[16px] font-semibold ${mainCls}`}>No approved sales uploads found</p>
          <p className={`mt-1 text-[13px] ${mutedCls}`}>No sales were approved in this date range for the selected outlet</p>
        </div>
      )}
