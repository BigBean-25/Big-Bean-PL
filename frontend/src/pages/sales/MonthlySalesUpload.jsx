...
const MonthlySalesUpload = () => {
  ...
  const fetchUploads = async () => {
    try {
      ...
      const response = await salesAPI.getReconciliations(params);
  ...
  const fetchReconciliationDetail = async (row) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || "Failed to fetch upload details");
      setSelectedUploadDetail({ ...row, errors: [], items: [] });
    }
  };

  const handleView = (row) => {
    fetchReconciliationDetail(row);
  };

  const handleDownloadTemplate = async () => {
    try {
      await salesAPI.downloadPetPoojaTemplate();
      toast.success("PetPooja sales template downloaded successfully");
    } catch (error) {
  ...
  const handleDownloadProcessed = (row) => {
    ...
    const safeFrom = String(row.date_from || "").replace(/[^a-zA-Z0-9\-_]/g, "_");
    const safeTo = String(row.date_to || "").replace(/[^a-zA-Z0-9\-_]/g, "_");
    const filename = `Monthly_Sales_Processed_${safeOutlet}_${safeFrom}_to_${safeTo}.xlsx`;
    salesAPI
  ...
  const handleApprove = async (row) => {
    try {
      ...
      toast.success("Monthly sales approved");
      await fetchUploads();
      if (selectedUpload?.id === row.id) await fetchReconciliationDetail(row);
    } catch (error) {
  ...
  const handleReject = async (row, reason) => {
    try {
      ...
      toast.success("Monthly sales rejected");
      await fetchUploads();
      if (selectedUpload?.id === row.id) await fetchReconciliationDetail(row);
    } catch (error) {
  ...
  const handleRollback = async (row) => {
    try {
      ...
      toast.success("Sales upload rolled back");
      setOpenMenuId(null);
  ...
  const handleUpload = async (event) => {
    ...
    if (!file) {
      toast.error("Please choose a file exported from PetPooja for this month");
      return;
    }
    ...
    try {
      ...
      const response = await salesAPI.uploadMonthly(payload);
      ...
      if (isMatched && errors === 0 && warnings === 0) {
        toast.success("Monthly sales uploaded and reconciled successfully.");
      } else if (errors > 0) {
        toast.error(`Monthly sales uploaded with ${errors} error${errors === 1 ? "" : "s"}. Review the error report before approving.`);
      } else if (warnings > 0) {
        toast.success(`Monthly sales uploaded with ${warnings} warning${warnings === 1 ? "" : "s"}. Review the error report.`);
      } else {
        toast.success("Monthly sales uploaded successfully.");
      }
  ...
  const summary = useMemo(() => {
    ...
    const totalNet = filteredUploads.reduce((s, r) => s + num(r.petpooja_net_sales), 0);
    const reconciled = filteredUploads.filter(
      (r) => r.reconciliation_status === "Matched" || r.reconciliation_status === "Approved"
    ).length;
    ...
    return { entries, totalNet, reconciled, needsAttention };
  }, [filteredUploads]);
      ...
      actions.push({
        ...
        onClick: () =>
          setConfirmAction({
            ...
            row,
            message:
              "This approved sales record will become eligible for accounting and P&L.",
          }),
      });
    }
      ...
      actions.push({
        ...
        onClick: () =>
          setConfirmAction({
            type: "reject",
            row,
            message: "Reject this Monthly Sales reconciliation?",
          }),
      });
    }
      ...
      actions.push({
        ...
        onClick: () =>
          setConfirmAction({
            ...
            row,
            message:
              "This removes the upload, reconciliation, item rows and uploaded file. This action cannot be undone.",
          }),
      });
    }
    return actions;
  ...
  return (
    ...
    >
      <PageHeader
        title="Monthly Sales"
        subtitle="Upload, reconcile and manage monthly PetPooja sales reports."
        isDark={isDark}
        actions={
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard
          ...
          subtitle="Visible monthly records"
          icon={FileSpreadsheet}
          color={primaryColor}
          bg={`${primaryColor}18`}
          isDark={isDark}
        ...
        <KpiCard
          ...
          subtitle="Sum of filtered uploads"
          icon={IndianRupee}
          color="#00CFE8"
          bg="#E6FAFD"
          isDark={isDark}
        ...
        <KpiCard
          ...
          subtitle="Matched / Approved records"
          icon={CheckCircle2}
          color="#28C76F"
          bg="#E9F9EF"
          isDark={isDark}
      ...
      <SectionCard isDark={isDark}>
        <div className="mb-5 flex items-center justify-between">
          <div>
            <h3 className={`text-lg font-semibold ${main}`}>Upload Monthly PetPooja Sales</h3>
            <p className={`mt-0.5 text-[13px] ${muted}`}>
              Upload the full calendar month's PetPooja Item Wise Sales Report.
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleUpload} className="space-y-5">
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
            ...
            <div>
              ...
              >
                ...
                {!file ? (
                  <>
                    ...
                    <p className={`mt-0.5 text-[13px] ${muted}`}>
                      or <label htmlFor="monthly-sales-file" className="cursor-pointer text-[#7367F0] hover:underline">click to browse</label>
                    </p>
                  </>
                ) : (
                  <div className="flex flex-col items-center justify-center gap-3 sm:flex-row sm:justify-between">
                    <div className="flex min-w-0 items-center gap-3">
                      <div
                        ...
                        style={{ backgroundColor: "#E9F9EF", color: "#28C76F" }}
                      >
              ...
              <p className={`mt-2 text-[12px] ${muted}`}>
                Month and date range will be detected automatically from the Excel file.
              </p>
            </div>
          </div>
          ...
          <div className="flex justify-end">
            ...
            >
              ...
              {uploading ? "Uploading..." : "Upload Monthly Sales"}
            </button>
          </div>
        </form>
      </SectionCard>
      ...
      <FilterBar isDark={isDark} title="Filters">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-5">
          ...
          <button
            ...
            className={`h-10 rounded-lg border px-3 text-[14px] font-medium transition ${getCardClass(
              isDark
            )} hover:bg-[#F8F7FA]`}
          >
            Clear Filters
      ...
      <SectionCard isDark={isDark}>
        <div className="flex items-center justify-between border-b px-5 py-4 sm:px-6">
          <div>
            <h3 className={`text-lg font-semibold ${main}`}>Monthly Sales History</h3>
            <p className={`text-[13px] ${muted}`}>Full-month sales uploads and reconciliation status.</p>
          </div>
        ...
        {loading ? (
            ...
            <table className="w-full min-w-[900px] border-collapse">
              <thead>
                <tr className={`border-b ${isDark ? "border-[#3B405A]" : "border-[#EBE9F1]"}`}>
                  {["Month", "Outlet", "Net Sales", "Cashbook", "Difference", "Allowed Difference", "Reconciliation", "Upload Status", "Uploaded On", "Actions"].map((h) => (
          ...
          <EmptyState
            icon={FileSpreadsheet}
            title="No Monthly Sales uploads yet"
            subtitle="Upload a full-month PetPooja sales report to begin reconciliation."
            isDark={isDark}
            action={
              canUpload ? (
                <button
          ...
          <TableWrapper isDark={isDark}>
            <table className="w-full min-w-[1000px] border-collapse">
              <thead>
                <tr className={`border-b ${isDark ? "border-[#3B405A]" : "border-[#EBE9F1]"}`}>
                  <th className={`px-4 py-3 text-left text-[12px] font-semibold uppercase tracking-wide ${main}`}>Month</th>
                  ...
                  return (
                    ...
                    >
                      <td className="px-4 py-3">
                        <p className={`text-[14px] font-semibold ${main}`}>
                          {formatMonthYear(row.date_from)}
                        </p>
                        <p className={`text-[12px] ${muted}`}>
            ...
            <div className={`flex items-start justify-between border-b px-6 py-5 ${isDark ? "border-[#3B405A]" : "border-[#EBE9F1]"}`}>
              <div>
                <h2 className={`text-xl font-bold ${main}`}>
                  {formatMonthYear(selectedUploadDetail.date_from)}
                </h2>
                <p className={`text-[14px] ${muted}`}>
                  {getOutletName(selectedUploadDetail)} — {selectedUploadDetail.batch_number}
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="overflow-y-auto p-6">
              <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                {[
                  ...
                  { label: "Net Sales", value: formatINR(selectedUploadDetail.petpooja_net_sales) },
                  { label: "Cashbook Total", value: formatINR(selectedUploadDetail.cashbook_total) },
                  { label: "Difference", value: formatINR(selectedUploadDetail.collection_difference) },
                ].map((k) => (
                  <div
                    key={k.label}
              ...
              <div className={`mt-5 grid grid-cols-1 gap-4 md:grid-cols-4`}>
                {[
                  { label: "Tolerance", value: formatINR(selectedUploadDetail.tolerance_amount) },
                  { label: "Final Collection", value: formatINR(selectedUploadDetail.petpooja_final_collection) },
                  { label: "Discount", value: formatINR(selectedUploadDetail.petpooja_discount) },
                  { label: "Tax", value: formatINR(selectedUploadDetail.petpooja_tax) },
                ].map((k) => (
                  <div key={k.label}>
                    <p className={`text-[12px] font-medium ${muted}`}>{k.label}</p>
              ...
              >
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3">
                  <div>
                    ...
                    <p className={`text-[15px] font-semibold ${main}`}>{formatINR(selectedUploadDetail.petpooja_final_collection)}</p>
                  </div>
                  <div>
                    <p className={`text-[12px] font-medium ${muted}`}>Cashbook Sales</p>
                    <p className={`text-[15px] font-semibold ${main}`}>{formatINR(selectedUploadDetail.cashbook_total)}</p>
                  </div>
                  <div>
                    <p className={`text-[12px] font-medium ${muted}`}>Difference</p>
                    <p className={`text-[15px] font-semibold ${main}`}>{formatINR(selectedUploadDetail.collection_difference)}</p>
                  </div>
                  <div>
                    <p className={`text-[12px] font-medium ${muted}`}>Allowed Tolerance</p>
                    <p className={`text-[15px] font-semibold ${main}`}>{formatINR(selectedUploadDetail.tolerance_amount)}</p>
                  </div>
                  <div className="sm:col-span-2 md:col-span-2">
                    <p className={`text-[12px] font-medium ${muted}`}>Result</p>
                    <div className="mt-1">
                      <StatusBadge
                        status={selectedUploadDetail.is_matched ? "Matched" : "Mismatched"}
                      />
                    </div>
                  </div>
                </div>
              ...
              <div className="mt-6">
                ...
                {selectedUploadDetail.items?.length > 0 ? (
                  <TableWrapper isDark={isDark} className="max-h-[260px]">
                    <table className="w-full min-w-[600px] border-collapse">
                      ...
                      <tbody>
                        {selectedUploadDetail.items.map((item, idx) => (
                  ...
                  <p className={`text-[13px] ${muted}`}>No item rows available.</p>
                )}
              </div>

              <div className="mt-6">
                <h4 className={`mb-3 text-[15px] font-semibold ${main}`}>Issues</h4>
                {selectedUploadDetail.errors?.length > 0 ? (
                  <div className="space-y-2">
                    {selectedUploadDetail.errors.map((err, idx) => (
                      <div
                        key={idx}
              ...
              <div className="flex flex-wrap gap-2">
                {hasPermission("item_sales", "can_export") && (
                  <>
                    <button
                      type="button"
                      onClick={() => handleDownloadOriginal(selectedUploadDetail)}
                    ...
                    <button
                      type="button"
                      onClick={() => handleDownloadProcessed(selectedUploadDetail)}
                      className={`h-9 rounded-lg border px-3 text-[13px] font-medium transition ${getCardClass(isDark)}`}
                    >
                      Processed
                    </button>
                ...
                {hasPermission("item_sales", "can_export") &&
                  (selectedUploadDetail.error_count > 0 || selectedUploadDetail.warning_count > 0) && (
                    <button
                      type="button"
                      onClick={() => handleDownloadErrorReport(selectedUploadDetail)}
                      className={`h-9 rounded-lg border px-3 text-[13px] font-medium transition ${getCardClass(isDark)}`}
                    >
                      Error Report
                    </button>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/50 p-4">
          <div className={`w-full max-w-md rounded-2xl border p-6 shadow-2xl ${getCardClass(isDark)}`}>
            <h3 className={`text-lg font-bold ${main}`}>
              {confirmAction.type === "approve" && "Approve Monthly Sales?"}
              {confirmAction.type === "reject" && "Reject Monthly Sales"}
              {confirmAction.type === "rollback" && "Rollback Monthly Sales Upload?"}
            </h3>
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button
                ...
                className={`h-10 rounded-lg px-4 text-[14px] font-semibold transition ${
                  confirmAction.type === "rollback" || confirmAction.type === "reject"
...
export default MonthlySalesUpload;
