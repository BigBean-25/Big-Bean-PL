...
const superRoles = ['Super Admin', 'Technical Admin', 'Developer'];
  ...
  return async (req, res, next) => {
    try {
      ...
      if (error.code === 'ER_NO_SUCH_TABLE') {
        return res.status(500).json({ success: false, message: 'role_permissions table is missing. Run database/role_permissions_migration.sql' });
      }
