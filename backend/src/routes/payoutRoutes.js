...
router.post('/online', protect, applyOutletScope, authorize('Admin', 'Super Admin'), async (req, res) => {
  try {
...
router.put('/online/:id', protect, applyOutletScope, authorize('Admin', 'Super Admin'), async (req, res) => {
  try {
    const fields = Object.keys(req.body);
    const values = Object.values(req.body);
...
router.delete('/online/:id', protect, applyOutletScope, authorize('Super Admin'), async (req, res) => {
  try {
...
router.post('/dine-in', protect, applyOutletScope, authorize('Admin', 'Super Admin'), async (req, res) => {
  try {
    const fields = Object.keys(req.body);
    const values = Object.values(req.body);
    const placeholders = fields.map(() => '?').join(', ');
...
router.put('/dine-in/:id', protect, applyOutletScope, authorize('Admin', 'Super Admin'), async (req, res) => {
  try {
    const fields = Object.keys(req.body);
    const values = Object.values(req.body);
    const setClause = fields.map(f => `${f} = ?`).join(', ');
...
router.delete('/dine-in/:id', protect, applyOutletScope, authorize('Super Admin'), async (req, res) => {
  try {
