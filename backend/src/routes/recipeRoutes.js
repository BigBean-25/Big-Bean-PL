...
router.get('/:id', protect, applyOutletScope, async (req, res) => {
  try {
    const scopeFilter = req.outletScope && !req.outletScope.all
      ? ` AND (for_outlet_id IS NULL OR for_outlet_id IN (${req.outletScope.outletIds.map(() => '?').join(',')}))`
      : '';
    const recipes = await query(`SELECT * FROM recipes WHERE id = ?${scopeFilter}`, [req.params.id, ...(req.outletScope?.outletIds || [])]);
...
router.post('/', protect, applyOutletScope, authorize('Admin', 'Super Admin'), async (req, res) => {
  try {
...
router.put('/:id', protect, applyOutletScope, authorize('Admin', 'Super Admin'), async (req, res) => {
  try {
...
router.delete('/:id', protect, applyOutletScope, authorize('Super Admin'), async (req, res) => {
  try {
    await query('DELETE FROM recipe_items WHERE recipe_id = ?', [req.params.id]);
