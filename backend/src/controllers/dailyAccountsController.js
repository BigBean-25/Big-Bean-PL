...
export const approveDailyCashExpense = async (req, res) => {
  try {
    ...
    const existing = await query('SELECT * FROM daily_cash_expenses WHERE id = ?', [id]);
    if (existing.length === 0) {
...
export const verifyDayClosing = async (req, res) => {
  try {
    ...
    const existing = await query('SELECT * FROM day_closings WHERE id = ?', [id]);
    if (existing.length === 0) {
      return res.status(404).json({ success: false, message: 'Day closing not found' });
    }
    if (Number(existing[0].submitted_by) === Number(req.user.id)) {
