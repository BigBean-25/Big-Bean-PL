...
export const getReconciliations = async (req, res) => {
  try {
    const { outlet_id, status, from_date, to_date } = req.query;
    ...
    const reconciliations = await query(
      `SELECT srb.*, psu.batch_number, psu.file_name, o.outlet_name,
...
export const getReconciliationById = async (req, res) => {
  try {
    ...
    const errors = await query(
      `SELECT * FROM sales_reconciliation_errors WHERE reconciliation_id = ?`,
      [id]
    );
    
    const items = await query(
      `SELECT * FROM petpooja_sales_items WHERE upload_id = ?`,
...
export const downloadErrorReport = async (req, res) => {
  try {
    ...
    summarySheet.addRow(['Date Range', `${reconciliation[0].upload_date_from} to ${reconciliation[0].upload_date_to}`]);
    summarySheet.addRow([]);
    summarySheet.addRow(['PetPooja Gross Sales', reconciliation[0].petpooja_gross_sales]);
    summarySheet.addRow(['PetPooja Net Sales', reconciliation[0].petpooja_net_sales]);
    summarySheet.addRow(['PetPooja Tax', reconciliation[0].petpooja_tax]);
    summarySheet.addRow(['Cashbook Total', reconciliation[0].cashbook_total]);
    summarySheet.addRow(['Difference', reconciliation[0].collection_difference]);
    summarySheet.addRow(['Status', reconciliation[0].is_matched ? 'Matched' : 'Mismatched']);
    
    // Errors sheet
    if (errors.length > 0) {
      const errorsSheet = workbook.addWorksheet('Errors');
      errorsSheet.addRow(['Error Type', 'Severity', 'Item Name', 'Expected', 'Actual', 'Difference', 'Message']);
...
export const approveSalesUpload = async (req, res) => {
  try {
    ...
    // Update P&L snapshot (if needed)
    // This would be implemented based on your P&L update logic
