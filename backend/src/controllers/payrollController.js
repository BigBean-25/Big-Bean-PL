...
export const updateEmployeeSalary = async (req, res) => {
  try {
    ...
    if (effectiveMonth !== existing[0].month || effectiveYear !== existing[0].year || Number(effectiveOutletId) !== Number(existing[0].outlet_id)) {
      ...
      // createEmployeeSalary rejects a second record for the same
      // month/year/outlet but this update path never re-checked it - only
      // the unique_month_outlet DB key caught it, surfacing as a raw 500
      // ER_DUP_ENTRY instead of a clean 400.
      const dup = await query(
        'SELECT id FROM employee_salary_monthly WHERE month = ? AND year = ? AND outlet_id = ? AND id != ?',
        [effectiveMonth, effectiveYear, effectiveOutletId, id]
      );
      if (dup.length > 0) {
        return res.status(400).json({
          success: false,
          message: 'Another salary record already exists for this month/year/outlet'
        });
      }
    }
...
export const verifyEmployeeSalary = async (req, res) => {
  try {
    ...
    // Verified salaries feed plCalculator.js (status='Verified' rows only) -
    // a transition on a record for an already-finalized month is still a
